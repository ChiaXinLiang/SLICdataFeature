...
class SLIC:
    ...
    def MakeSmallImage(self,layer):
        n,m = self.img.shape[0:2] 

        clusterNumber = self.clusters.reshape(n*m,1)
        maxCluster = int(max(clusterNumber))
        layer = int(layer)
        # print(maxCluster,layer)
        

        centers = []
        for i in range(0,maxCluster,layer):
            interCenter = 0
            for j in range(0,layer):
                interCenter = interCenter + self.centers[i+j]
            interCenter = interCenter/layer
            centers.append(interCenter)
        maxCluster = maxCluster/layer
        clusterNumber = clusterNumber/layer
        clusterNumber = [math.floor(i) for i in clusterNumber]



        allpixel = []
        for j in range(0,int(maxCluster)):
            superPixelPiece = 255*np.ones((n,m,3), np.int8)
            superPixelPiece1 = 255*np.ones((2*layer*self.step,2*layer*self.step,3), np.int8) 
            ...
            xlow = int(centers[j][4]) - layer*self.step 
            xhigh = int(centers[j][4]) + layer*self.step
            ylow = int(centers[j][3]) - layer*self.step
            yhigh = int(centers[j][3]) + layer*self.step
            ...
            if maxRow < 2*layer*self.step:
                superPixelPiece1[maxRow:(2*layer*self.step+1),:]=[255,255,255]
            if maxCol < 2*self.step*layer:
                superPixelPiece1[:,maxCol:(2*layer*self.step+1)] =[255,255,255]
    ...
    def MakeSmallGreyImg(self,layer):
